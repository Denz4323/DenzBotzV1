...
mentions(`「 ANTILINK 」\n\n@${sender.split('@')[0]} Lu mengirim link group, maaf bot akan kick lu dari grup`, [sender])
await sleep(3000)
...
if (command === 'buatroom') {
if (cekPesan("id", sender) !== null) return reply("Lu Sedang Didalam roomchat ketik *#stopchat* untuk menghapus sesi chat.")
...
reply("Lu Sedang di dalam sesi room chat, menunggu konfirmasi dari penerima.")
}
}
...
var text_tersambung =`*Seseorang Mengajak Chating*\n\n*Dari:* Rahasia\n\nSilahkan klik button ya bre jika ingin menghubungkan chat *ANONYMOUS*`
...
reply("Url YouTube nya mana bre?")
}
}
...
var xx = ytmp4.result
if (ytmp4.status == 500) return reply('*Link yg lu berikan tidak valid*\n*Silahkan kirim url yg valid&benar*')
...
var xx = ytmp3.result
if (ytmp3.status == 500) return reply('*Link yg lu berikan tidak valid*\n*Silahkan kirim url yg valid&benar*')
...
reply("Username npmnya mana bre?")
}
}
...
fs.writeFileSync(PathAuto + sender.split("@")[0] + ".json", JSON.stringify(deposit_object, null, 2))
reply("*Silahkan kirim ID free fire lu*")
} else {
reply("id ff nya mana bre?")
}
}
...
reply('Lu terdeteksi spam bot tanpa jeda, lakukan perintah setelah 5 detik')
}
...
switch(command) {
case 'verify':{
if (cekUser("id", sender) !== null) return reply('Lu sudah terdaftar !!')
...
if (!isLinks) return reply('Link yang lu berikan tidak valid')
...
reply('Chat aja bre, ga usah malu')
}
break
case 'room':{
if (!isOwner) return reply(mess.OnlyOwner)
...
break
case 'stopchat':
if (cekPesan("id", sender) == null) return reply(`Lu sedang tidak didalam roomchat, Silahkan buat room dengan contoh dibawah ini.\n\n*Example:*\n#menfess num|nama|pes\n\n*Contoh:*\n#menfess 628xxx|bot|hai\n\n*Note:*\n628xxxxxx - benar (✅)\n+62 8xx xxxx xxxx - salah (❌)`)
...
reply('Pesan Menfess lu sudah dikirim, Sedang menunggu untuk diterima.')
if (isGroup) return reply("Pesan menfess lu sudah dikirim.")
}
break
case 'request': {
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
...
var bg_denz = '6285604500380@s.whatsapp.net'
conn.sendMessage(bg_denz, {text: teks + teks1, mentions:[sender]}, {quoted:msg})
...
conn.sendMessage(from, { document: { url: url }, fileName: filename, mimetype: 'application/zip' }, { quoted: msg }).catch((err) => reply('Maaf link github yang lu berikan di private, dan tidak bisa di jadikan file'))
}
break
...
.then(data => {
var opt = { packname: 'DenzBotz', author: 'By Denz Gembel' }
...
reply("[❗] _Tunggu bre, sedang proses!_")
...
conn.sendMessage(from, { image: { url: random }, caption: `Nih bre` }, { quoted: msg })
}
break
case 'wallpaperinori':{
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
...
conn.sendMessage(m.chat, { image: { url: random }, caption: `Nih bre` }, { quoted: fakekirbotz })
}
break
case 'jadibot': {
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
...
var dev = '6285604500380@s.whatsapp.net'
